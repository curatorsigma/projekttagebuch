...
use crate::{config::Config, db::{get_person, get_project, remove_members, update_project_members, DBError}, types::{HasID, Person, Project, UserPermission}};
...
pub(super) async fn add_member_to_project(config: Arc<Config>, requester: &Person<HasID>, new_member_name: &str, project_id: i32) -> Result<(Person<HasID>, Project<HasID>), AddMemberError> {
        ...
        match update_project_members(config.pg_pool.clone(), &project).await {
            Ok(()) => {
                info!("Added {} to {} as User; request made by {}.", new_member.name, project.name, requester.name);
...
/// Or the appropriate error
pub(super) async fn remove_member_from_project(config: Arc<Config>, requester: &Person<HasID>, remove_member_name: &str, project_id: i32) -> Result<(Person<HasID>, Project<HasID>), RemoveMemberError> {
        // the permission to do this depends on the project, so we need to get that before checking
        // permission
        let mut project = match get_project(config.pg_pool.clone(), project_id).await {
            Ok(Some(x)) => x,
        ...
        // - send command to matrix
        // - rollback if matrix answers with error
        match remove_members(config.pg_pool.clone(), project.project_id(), &[&remove_member]).await {
            Ok(()) => {
                info!("Removed {} from {} as User; request made by {}.", remove_member.name, project.name, requester.name);
